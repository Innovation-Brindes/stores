...
export const ConfirmarCompra = ({ id, dataInfoClientes, data, dadosPix }) => {
  ...
  const [boleto, setBoleto] = useState((typeof window !== "undefined" && localStorage.getItem("boleto")) || false)
  ...
  async function pixSubmit() {
    ...
    const params = {
      ...
      nome: nomeCliente,
      chave_emp: chaveEmp,
      substituicao_tributaria: parseFloat(valores.substituicaoTributaria).toFixed(2).toString().replace(".", ""),
      valor_frete: parseFloat(valorFreteAmostra).toFixed(2).toString().replace(".", ""),
    }

    const response = await baseURL.post("/pedido/envia-dados-pagamento", params)
  ...
  async function boletoSubmit() {
    ...
    const params = {
      ...
      tipo_pagamento: paymentType,
      substituicao_tributaria: parseFloat(valores.substituicaoTributaria).toFixed(2).toString().replace(".", ""),
      valor_entrega: parseFloat(valorFreteAmostra).toFixed(2).toString().replace(".", ""),
    }
  ...
  async function getIpUser() {
    ...
    const response = await baseURL.post("/pedido/atualiza-usuario-aprovacao-pagamento", params)
  }

  async function getDataPagamentos() {
    try {
      const { data } = await baseURL.get(`/pedido/busca-info-itens-pagamento/${id}`)

      const PRAZO_ENTREGA = data.map((item) => item.PRAZO_ENTREGA)
      const QTD = data.map((item) => parseInt(item.QTDNEG))
      const VALORUNIT = data.map((item) => Number(item.VALOR_UNIT).toFixed(2).replace(".", ""))
      const DESCRICAO = data.map((item) => item.DESCRICAO.substring(0, 10))
      const NUNOTA = data.map((item) => item.NUNOTA)
      const VALOR_FRETE = data.map((item) => Number(item.VALOR_FRETE).toFixed(2).replace(".", ""))
      ...
      const SUBSTITUICAO_TRIBUTARIA = data.map((item) => item.VLR_SUB_TRIBUTARIA)
  ...
  async function getDataInfoClientes() {
    try {
      ...
      const VLRFRETE = data.map((item) => Number(item.VALOR_FRETE).toFixed(2).replace(".", ""))
  ...
  return (
    <>
      ...
      <Flex
        ...
        width="100%"
      >
        <Header isMinWidthMedium={isMinWidthMedium} pedCompra={pedCompra} nunota={nunota} />
        ...
        <CardContainer dataInfoClientes={dataInfoClientes} textFrete={textFrete} />

        <ChakraProvider>
          {!verifyIsAproved && !pedidoPago && !method && (
            <TableContainer w="100%" {...(isMinWidthMedium && { display: "none" })}>
              <Table
                variant="simple"
              ...
              >
                ...
                <Tbody>
                  <Card cardProduct data={data} setIsChecked={setCheckedRow} isChecked={checkedRow} />
                </Tbody>
              </Table>
            </TableContainer>
          )}
        ...
        {!verifyIsAproved && !!isMinWidthMedium && !method && (
          <>
            <Flex flexDir="column" gap={8} width="100%">
              <Card isMobile data={data} setIsChecked={setCheckedRow} isChecked={checkedRow} />
            </Flex>
          </>
        )}
        ...
        {!loading && !isMinWidthMedium && (
          <ResumoPedido valores={valores} valorTotalItens={valorTotalItens} valorFreteResumo={valorFreteAmostra} />
        )}
        ...
        {paymentType.includes("BOLETO") && boleto === "false" && (
          <>
            <TimerConfirmar
              label={
                !verifyIsAproved ? "O tempo para você aprovar o pedido acaba em:" : "O tempo para você pagar acaba em:"
              }
            />
